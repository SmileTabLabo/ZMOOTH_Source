...
struct rockchip_connector_funcs {
	...
	int (*get_edid)(struct display_state *state);	
	 /*
	 * for reading panel id,configure mipi host and phy simply so that reading and writing command can send succeessfully.
	 */
	void (*pre_prepare)(struct display_state *state);
	/*
