...
struct battery_priv {
	...
	struct gpio_desc wacom_stop_gpio;
	struct gpio_desc wacom_RST_gpio;
...
static void rk818_bat_not_first_pwron(struct battery_priv *di)
{
	...
	BAT_INFO("pre_soc:%d voltage:%d charge state:%d \n",pre_soc,voltage,rk818_bat_get_usb_state(di));
	if (di->is_halt) {
	...
	} else if ((pre_soc == 0) && (voltage >= ZERO_MIN_VOLTAGE)&&(rk818_bat_get_usb_state(di)==NO_CHARGER)) {
		if (now_cap < 0)
			now_cap = 0;
		rk818_bat_init_capacity(di, now_cap);
		pre_cap = di->remain_cap;
...
{
	u8 usb_ctrl;
	printf("\n %s set current up to %d ",__func__,input_current);
	usb_ctrl = rk818_bat_read(di, USB_CTRL_REG);
	usb_ctrl &= ~INPUT_CUR_MSK;
...
static void rk818_bat_charger_setting(struct battery_priv *di, int charger)
{
	...
	if (old_charger != charger) {
		if (charger == NO_CHARGER) {
			BAT_INFO("NO_CHARGER\n");
			if(di->chg_led_is_valid)dm_gpio_set_value(&di->chg_led,0);
			rk818_bat_set_current(di, ILIM_450MA);
		} else if (charger == USB_CHARGER) {
			BAT_INFO("USB_CHARGER\n");
			if(di->chg_led_is_valid)dm_gpio_set_value(&di->chg_led,1);
			rk818_bat_set_current(di, ILIM_450MA);
...
#ifdef CONFIG_LCD_ID_DET
struct gpio_desc ID1_gpio;
struct gpio_desc ID2_gpio;
static bool get_ID_dtsi=0;
u8 get_LCD_ID_value(void)
{
	u8 value=0;
	...
	if(get_ID_dtsi)
	{
		value=(dm_gpio_get_value(&ID1_gpio)<<1)^(dm_gpio_get_value(&ID2_gpio));		
		printf("\n %s ID1_gpio.gpio state:%d  ID2_gpio.gpio  state:%d  \n",__func__,dm_gpio_get_value(&ID1_gpio),dm_gpio_get_value(&ID2_gpio));

	}
	return value;
}
EXPORT_SYMBOL(get_LCD_ID_value);
...
static int rk818_fg_ofdata_to_platdata(struct udevice *dev)
{
	...
	ret = gpio_request_by_name_nodev(dev_ofnode(dev), "charger_led_gpio",
					 0, &di->chg_led, GPIOD_IS_OUT);
	if (!ret) {
		di->chg_led_is_valid = 1;
		debug("charger_led is valid\n");
	} else {
		debug("charger_led is invalid, ret=%d\n", ret);
	}


	ret = gpio_request_by_name_nodev(dev_ofnode(dev), "wacom_stop_gpio",
					 0, &di->wacom_stop_gpio, GPIOD_IS_OUT);
	if (!ret) {
		dm_gpio_set_value(&di->wacom_stop_gpio,0);
		printf("wacom_stop_gpio is valid\n");
		printf("wacom_stop_gpio is %d\n",dm_gpio_get_value(&di->wacom_stop_gpio));
	} else {
		printf("wacom_stop_gpio is invalid, ret=%d\n", ret);
	}


	ret = gpio_request_by_name_nodev(dev_ofnode(dev), "wacom_RST_gpio",
					 0, &di->wacom_RST_gpio, GPIOD_IS_OUT);
	if (!ret) {
		dm_gpio_set_value(&di->wacom_RST_gpio,0);
		printf("wacom_RST_gpio is valid\n");
		printf("wacom_RST_gpio is %d\n",dm_gpio_get_value(&di->wacom_RST_gpio));
	} else {
		printf("wacom_RST_gpio is invalid, ret=%d\n", ret);
	}


#ifdef CONFIG_LCD_ID_DET
	ret = gpio_request_by_name_nodev(dev_ofnode(dev), "ID1_det_gpio",
					 0, &ID1_gpio, GPIOD_IS_IN);
	if (!ret) {

		get_ID_dtsi=1;
		printf("ID1_gpio is %d\n",dm_gpio_get_value(&ID1_gpio));
	} else {
		printf("ID1_gpio is invalid, ret=%d\n", ret);
	}


	ret = gpio_request_by_name_nodev(dev_ofnode(dev), "ID2_det_gpio",
					 0, &ID2_gpio, GPIOD_IS_IN);
	if (!ret) {
		get_ID_dtsi=1;
		printf("ID2_gpio is %d\n",dm_gpio_get_value(&ID2_gpio));
	} else {
		printf("ID2_gpio is invalid, ret=%d\n", ret);
	}
#endif
