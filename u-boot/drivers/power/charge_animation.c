...
static int charge_animation_ofdata_to_platdata(struct udevice *dev)
{
	...
	printf("  mode: uboot=%d, android=%d; exit: soc=%d%%, voltage=%dmv;\n"
...
static int charge_animation_show(struct udevice *dev)
{
	...
	int boot_mode;
	int first_poll_fg = 1;
	bool android_charge_batt_check=0;
	bool first_round =1;
	bool mShowAnima=true;
/*
 * Check sequence:
 *
 ...
 */
	...
	if (pdata->android_charge) {
		...
		printf("Android charge mode\n");
		 android_charge_batt_check=1;
	}

	/* Not enable U-Boot charge, exit */
	if (!pdata->uboot_charge) {
		printf("Exit charge: due to not enable uboot charge\n");
		if(!android_charge_batt_check)return 0;
	}

	voltage = fuel_gauge_get_voltage(fg);
	if (voltage < 0) {
		printf("  get voltage failed: %d\n", voltage);
		if(!android_charge_batt_check)return -EINVAL;
		else return 0;
	}
	...
	else if (voltage <= pdata->screen_on_voltage + 50) {
		screen_on = false;
		ever_lowpower_screen_off = true;
		charge_show_bmp(NULL);
	}
	else if(android_charge_batt_check)
	{
		screen_on = false;
		ever_lowpower_screen_off = true;
		charge_show_bmp(NULL);
...
show_images:
		...
		if (screen_on) {
			...
			printf("  first_round %d ... \n", first_round);
			if(!first_round)system_suspend_enter(dev);
			else first_round=0;
		}
		...
		if (key_state == KEY_PRESS_DOWN) {
			...
			if(android_charge_batt_check)
				{
					printf("  Enter Android boot charging...\n");
					return 0;
				}
		...
		if(android_charge_batt_check)
			{
				printf("  voltage=%dmv, threshold voltage=%dmv\n",voltage, pdata->exit_charge_voltage);
				if (voltage < pdata->exit_charge_voltage) {
					printf("  Low power, unable to android boot charging...\n");
				}
				else
				{
					printf("  Return to Android boot charging...\n");
					return 0;
				}
			}
		/* Step5: Exit by ctrl+c */
		if (ctrlc()) {
			if (voltage >= pdata->screen_on_voltage || mShowAnima)
				charge_show_logo();
