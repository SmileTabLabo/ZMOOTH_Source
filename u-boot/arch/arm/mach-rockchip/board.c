...
static int rockchip_set_cust1(void)
{
...
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(12, serialno_str, (VENDOR_SN_MAX-1));
	if (ret > 0) {
		env_set("cust1#", serialno_str);
	} else {
#endif
...
static int rockchip_set_cust2(void)
{
	char serialno_str[VENDOR_SN_MAX];
	int ret = 0, i;
	u8 cpuid[CPUID_LEN] = {0};
	...
	memset(serialno_str, 0, VENDOR_SN_MAX);
#ifdef CONFIG_ROCKCHIP_VENDOR_PARTITION
	ret = vendor_storage_read(13, serialno_str, (VENDOR_SN_MAX-1));
	if (ret > 0) {
		env_set("cust2#", serialno_str);
	} else {
#endif
#ifdef CONFIG_ROCKCHIP_EFUSE
		struct udevice *dev;
