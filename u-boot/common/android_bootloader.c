...
#define ANDROID_ARG_cust1no "androidboot.cust1no="
#define ANDROID_ARG_cust2no "androidboot.cust2no="
				      ...
				      const char *extra_args)
{
	...
	char *env_cmdline, *cmdline, *rootdev_input, *serialno, *cust1no, *cust2no;
	char *allocated_suffix = NULL;
	char *allocated_serialno = NULL;
	char *allocated_rootdev = NULL;
	char *allocated_cust1no = NULL;
	char *allocated_cust2no = NULL;
	...
	cust1no = env_get("cust1#");
	if (cust1no) {
		allocated_cust1no = malloc(strlen(ANDROID_ARG_cust1no) +
					  strlen(cust1no) + 1);
		memset(allocated_cust1no, 0, strlen(ANDROID_ARG_cust1no) +
				strlen(cust1no) + 1);
		strcpy(allocated_cust1no, ANDROID_ARG_cust1no);
		strcat(allocated_cust1no, cust1no);
		*(current_chunk++) = allocated_cust1no;
	}

	cust2no = env_get("cust2#");
	if (cust2no) {
		allocated_cust2no = malloc(strlen(ANDROID_ARG_cust2no) +
					  strlen(cust2no) + 1);
		memset(allocated_cust2no, 0, strlen(ANDROID_ARG_cust2no) +
				strlen(cust2no) + 1);
		strcpy(allocated_cust2no, ANDROID_ARG_cust2no);
		strcat(allocated_cust2no, cust2no);
		*(current_chunk++) = allocated_cust2no;
	}
