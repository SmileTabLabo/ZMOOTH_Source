...
static int dw9714_read_reg(struct i2c_client *client, u8 reg, u8 *val)
{
	...
	unsigned int *cur_pos)
{
...
#if 0
	ret = dw9714_read_reg(client, 0x03, &msb);
	if (ret != 0)
		goto err;
	ret = dw9714_read_reg(client, 0x04, &lsb);
	if (ret != 0)
		goto err;
#else
	...
	if (ret != 0)
		goto err;
#endif
	abs_step = (((unsigned int)(msb & 0x3U)) << 8U) |
		   ((unsigned int)lsb);
	...
	*cur_pos = abs_step;
	dev_info(&client->dev, "%s: get position %d\n", __func__, *cur_pos);
	return 0;
...
//	int loop_time = 0;
	...
	dev_info(&client->dev, "set position (0x%04x)\n", position);
	...
	while (loop_time < 10)
	{
		dw9714_read_reg(client, 0x05, &status);
		status = status& 0x01; //get reg 05 status
		dev_info(&client->dev, "dw9714V/P 0x05 status:%x", status);
		...
		loop_time++;
		usleep_range(2000, 2000+10); //delay 1ms
	}

	if (loop_time >= 10)
	{
		 dev_info(&client->dev, "watting 0x05 flag timeout!");
		return -1;
	}
	dw9714_write_msg(client, 0xED, 0xAB);//Advanced Mode
	ret = dw9714_write_msg(client, 0x03,((position>>8)&0x03));
	ret |= dw9714_write_msg(client, 0x04,(position&0xff));
	dw9714_write_msg(client, 0xDF, 0x5B);//Normal Mode
#endif
			...
			const struct i2c_device_id *id)
{
	...
	//init function
	dw9714_write_msg(client, 0xED, 0xAB);//Advanced Mode
	dw9714_write_msg(client, 0x02, 0x01);//Power down
	dw9714_write_msg(client, 0x02, 0x00);//active
	usleep_range(1000, 1000+10); //delay 1ms
	dw9714_write_msg(client, 0x06, 0x84);//SAC3 mode, NRC time:32.11ms
	dw9714_write_msg(client, 0x07, 0x01);//Set Tvib (PRESC[1:0] )
	dw9714_write_msg(client, 0x08, 0x49);//Set Tvib (SACT[6:0] )
	dw9714_write_msg(client, 0x0A, 0x03);//NRC Mode
#endif
