...
static int __gc5035_power_on(struct gc5035 *gc5035)
{
	...
	{
		//printk(" %s reset_gpio:%d --%d-- !!\n",__func__,gpiod_get_value(gc5035->reset_gpio),__LINE__);
	...
	if (!IS_ERR(gc5035->reset_gpio))gpiod_set_value_cansleep(gc5035->reset_gpio, 1);
	

	usleep_range(500, 1000);
	if (!IS_ERR(gc5035->pwdn_gpio))
		{
			//printk(" %s pwdn_gpio:%d --%d-- \n",__func__,gpiod_get_value(gc5035->pwdn_gpio),__LINE__);
	...
	usleep_range(1000, 1100);
	if (!IS_ERR(gc5035->reset_gpio))//printk(" %s reset_gpio:%d --%d-- !!\n",__func__,gpiod_get_value(gc5035->reset_gpio),__LINE__);
	if (!IS_ERR(gc5035->pwdn_gpio))//printk(" %s pwdn_gpio:%d --%d-- \n",__func__,gpiod_get_value(gc5035->pwdn_gpio),__LINE__);
	return 0;
...
static int gc5035_set_test_pattern(struct gc5035 *gc5035, int value)
{
	...
	dev_info(&gc5035->client->dev, "Test Pattern END!!\n");
	return ret;
}
