...
extern int rk818_get_temp(void);
static u8 last_input_current=0xC0; 
...
struct charger_platform_data {
	...
	u32 tc_count;
	int vcc5v_typc;
	enum of_gpio_flags vcc5v_typc_flags;
};
...
static struct rk818_charger *rk818_cg;
...
int rk818_get_last_cur(void)
{
	return last_input_current;
}
EXPORT_SYMBOL_GPL(rk818_get_last_cur);
...
static void rk818_cg_set_otg_power(struct rk818_charger *cg, int state)
{
	...
	case USB_OTG_POWER_ON:
		if (cg->otg_pmic5v) {
			...
			if(gpio_is_valid(cg->pdata->vcc5v_typc))gpio_set_value(cg->pdata->vcc5v_typc,1);
	...
	case USB_OTG_POWER_OFF:
		if (!cg->otg_pmic5v) {
			...
			if(gpio_is_valid(cg->pdata->vcc5v_typc))gpio_set_value(cg->pdata->vcc5v_typc,0);
...
static int rk818_cg_parse_dt(struct rk818_charger *cg)
{
	...
	pdata->vcc5v_typc = of_get_named_gpio_flags(np, "VCC5V0_TYPEC0_gpios",0, &pdata->vcc5v_typc_flags);
	if (!gpio_is_valid(pdata->vcc5v_typc)) {
		dev_err(dev, "Can not read property vcc5v_typc gpio\n");
	}
	else {
		ret = devm_gpio_request_one(dev, pdata->vcc5v_typc,
				    GPIOF_DIR_OUT, "VCC5V0_TYPEC0_gpios");
		if (ret < 0) pdata->vcc5v_typc=-1; 
		}
...
notify_fail:
 ...
 void rk818_charger_shutdown_P(void)
{
	struct rk818_charger *cg;
...
EXPORT_SYMBOL_GPL(rk818_charger_shutdown_P);
