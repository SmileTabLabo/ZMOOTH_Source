...
static int tmp_test = 100;
static int air_enable=0;
static int disCCCV_EN=0;
#define INPUT_CUR850MA         (0x02)
module_param_named(dbg_level, dbg_enable, int, 0644);
module_param_named(tmp_level, tmp_test, int, 0644);
module_param_named(air_level, air_enable, int, 0644);
module_param_named(CCCV_level, disCCCV_EN, int, 0644);
extern int rk818_get_last_cur(void);

static struct rk818_battery *rk818_di_p;
//static unsigned int temp_average_value=0;
//static u8 average_time=0;
//static unsigned int old_average_value=0;
...
struct rk818_battery {
	...
	int chg_led;
	enum of_gpio_flags chg_led_flags;
};

#define DIV(x)	((x) ? (x) : 1)

static int RK818_reset_gpios;
static enum of_gpio_flags RK818_reset_gpios_flags;


int rk818_set_reset(bool val)
{
	printk("%s \n",__func__);
	if(gpio_is_valid(RK818_reset_gpios))
	{
		printk("%s set reset pin:%d from %d to %d \n",__func__,RK818_reset_gpios,gpio_get_value(RK818_reset_gpios),val);
		gpio_direction_output(RK818_reset_gpios,1);
		msleep(200);
		printk("%s set reset pin:%d from %d to 0 \n",__func__,RK818_reset_gpios,gpio_get_value(RK818_reset_gpios));
		gpio_direction_output(RK818_reset_gpios,0);
	}
	return 1;
}
EXPORT_SYMBOL_GPL(rk818_set_reset);

int rk818_get_temp(void)
{
	return tmp_test;
}
EXPORT_SYMBOL_GPL(rk818_get_temp);
...
static void rk818_bat_enable_gauge(struct rk818_battery *di)
{
	...
	buf = rk818_bat_read(di, RK818_BAT_CTRL_REG);
	buf &= ~0x7;
	buf |= 0x2;//max 4A
	rk818_bat_write(di, RK818_BAT_CTRL_REG, buf);


	buf=0x48;//9 `c
//	rk818_bat_write(di, RK816_BAT_LTS_TS_REG, buf);
	buf=0x11;// 47`c
//	rk818_bat_write(di, RK816_BAT_HTS_TS_REG, buf);
}
				      ...
				      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		if(old_temp!=-100)val->intval = old_temp;
		else val->intval = di->temperature;
		if (di->pdata->bat_mode == MODE_VIRTUAL)
...
//	chrg_ctrl3 &= ~CHRG_TIMER_CCCV_EN;
	chrg_ctrl3 |= CHRG_TIMER_CCCV_EN;
	chrg_ctrl3 |= CHRG_TIMER_Trikl_EN;
...
static bool curr_set =0;
static bool air_set =0;
static bool disCCCV_set=0;
static 	u8 out_range=0;
...
/*
	printk("Batt temperature:%d BAT_LTS_TS1:0x%x  RK816_BAT_HTS_TS_REG:0x%x !!!!!!!\n",di->temperature, rk818_bat_read(di,RK816_BAT_LTS_TS_REG), rk818_bat_read(di,RK816_BAT_HTS_TS_REG));*/
	if(gpio_is_valid(di->chg_led))
	{
		if(di->usb_in||di->ac_in)gpio_set_value(di->chg_led,1);
		else gpio_set_value(di->chg_led,0);
	}
	if(!air_enable)
	{
	
		if(rk818_get_temp()<10)
		{
			...
			if(out_range>1)
			{
				...
				usb_ctrl = rk818_bat_read(di, RK818_USB_CTRL_REG);
					...
					usb_ctrl |= INPUT_CUR80MA;
					if(usb_ctrl)rk818_bat_write(di, RK818_USB_CTRL_REG, usb_ctrl);
				//	printk("set INPUT_CUR80MA!\n");
				if(!curr_set)printk("The NTC temperature is Out of range ! set input current 80mA ! \n");
				curr_set =1;
			}
			else 
			{
				if(out_range<10)out_range++;
			}
		}
		else if((10<rk818_get_temp())&&(curr_set ))
		{

			if(rk818_get_last_cur())
			{
				usb_ctrl = rk818_get_last_cur();
				if(usb_ctrl)rk818_bat_write(di, RK818_USB_CTRL_REG, usb_ctrl);
				out_range=0;
				if(curr_set)printk("The NTC temperature is OK ! Reset input current ! \n");
				curr_set=0;
			} 
		}
		else out_range=0;
	...
	if(air_enable)
	{
		usb_ctrl = rk818_bat_read(di, RK818_USB_CTRL_REG);
			usb_ctrl &= ~INPUT_CUR_MSK;
			usb_ctrl |= INPUT_CUR850MA;//INPUT_CUR80MA;
			if(usb_ctrl)rk818_bat_write(di, RK818_USB_CTRL_REG, usb_ctrl);
		//	printk("set INPUT_CUR80MA!\n");
			if(!air_set)printk("The Flight transport mode ! set input current 80mA ! \n");
			air_set =1;
	...
	else
	{
		...
		{
			usb_ctrl &= ~INPUT_CUR_MSK;
			usb_ctrl |=  rk818_get_last_cur();//INPUT_CUR1750MA;
			if(usb_ctrl)rk818_bat_write(di, RK818_USB_CTRL_REG, usb_ctrl);
			air_set=0;
			if(air_set)printk("The Flight transport mode Off ! Reset input current ! \n");
		} 
	}
	...
	if(disCCCV_EN)
	{
		...
		chrg_ctrl3 &= ~CHRG_TIMER_CCCV_EN;
		rk818_bat_write(di, RK818_CHRG_CTRL_REG3, chrg_ctrl3);
		if(!disCCCV_set)printk("Disable CCCV  ! \n");
		disCCCV_set =1;
	}
	else
	{
		...
		{	
			...
			chrg_ctrl3 |= CHRG_TIMER_CCCV_EN;
			rk818_bat_write(di, RK818_CHRG_CTRL_REG3, chrg_ctrl3);
			if(disCCCV_set)printk("Enable CCCV  ! \n");
			disCCCV_set =0;
		}
	}
...
static void rk818_bat_update_temperature(struct rk818_battery *di)
{
	...
	if (ntc_size) {
		...
		//	di->temperature = VIRTUAL_TEMPERATURE;
			 ...
			 */
			if (abs(temp - old_temp) > 300)
				temp = old_temp;
			else
				old_temp = temp;

			di->temperature = temp-50;
		//	if(460<di->temperature)di->temperature=460;
			if(90>di->temperature)di->temperature=90;
			printk("<%s>. temperature = %d\n",
			    __func__, di->temperature);
			rk818_bat_temp_notifier_callback(di->temperature / 10);
			
			if((tmp_test!=8)&&(tmp_test!=18))
			{
				///if((di->temperature<100)||(450<di->temperature))tmp_test=5; 
				//else 
				tmp_test=100; 
			}
		}
...
/*			if(average_time==0)
				{
					if(old_average_value==0)old_average_value=temp;
					temp_average_value=temp;
					average_time++;

				}
			else
				{
					temp_average_value=temp_average_value+temp;
					average_time++;
					if(average_time>9)
						{
							temp_average_value=temp_average_value/10;
							old_average_value=temp_average_value;
							average_time=0;
							temp=old_average_value;
						}
				}
		printk("\nold_average_value=%d temp_average_value:%d old_temp:%d\n",old_average_value,temp_average_value,old_temp);
			old_temp= temp;
			di->temperature =temp;
			printk("<%s>. temperature = %d  \n",
			    __func__, di->temperature);
			...
			if((tmp_test!=8)&&(tmp_test!=18))
			{
				if((di->temperature<150)||(400<di->temperature))tmp_test=5; 
				else tmp_test=100; 
			}	
		}
	}
...
static int rk818_bat_parse_dt(struct rk818_battery *di)
{
	...
	di->chg_led = of_get_named_gpio_flags(np, "charger_led_gpio",0, &di->chg_led_flags);
	if (!gpio_is_valid(di->chg_led)) {
		dev_err(dev, "Can not read property chg_led gpio\n");
	}
	else {
		ret = devm_gpio_request_one(dev, di->chg_led,
				    GPIOF_DIR_OUT, "charger_led_gpio");
		if (ret < 0) di->chg_led=-1; 
		}


	RK818_reset_gpios = of_get_named_gpio_flags(np, "RK818_reset_gpios",0, &RK818_reset_gpios_flags);
	if (!gpio_is_valid(RK818_reset_gpios)) {
		dev_err(dev, "Can not read property RK818_reset_gpios gpio\n");
	}
	else {
		ret = devm_gpio_request_one(dev, RK818_reset_gpios,
				    GPIOF_DIR_OUT, "RK818_reset_gpios");
		gpio_direction_output(RK818_reset_gpios,0);
		if (ret < 0) RK818_reset_gpios=-1; 
		}
...
static int rk818_battery_resume(struct platform_device *dev)
{
 ...
 void rk818_battery_shutdown_P(void)
{
	u8 cnt = 0;
	struct rk818_battery *di ;
	di=rk818_di_p;
...
EXPORT_SYMBOL_GPL(rk818_battery_shutdown_P);
