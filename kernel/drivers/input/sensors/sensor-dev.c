...
//	printk("%s cmd:%d \n",__func__,cmd);
	...
	case GSENSOR_IOCTL_APP_SET_RATE:
		...
		break;
	case GSENSOR_IOCTL_GET_ACCEL_ID:
	if (sensor_cali_data.accel_ID) result=  sensor_cali_data.accel_ID;
	return result;
	break;
	default:
		break;
...
int sensor_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	if(type==SENSOR_TYPE_ACCEL)sensor_cali_data.accel_ID=(int)sensor->i2c_id->driver_data;
	return result;
