...
extern int Imt_LCD_DeviceInfo(void);
static bool x_reverse=0;
static bool y_reverse=0;
static bool xy_reverse=0;
...
#if GTP_ICS_SLOT_REPORT
#ifdef CONFIG_GT9XX_M102_portrait
	x=4096-x;
#endif
...
static void goodix_ts_work_func(struct work_struct *work)
{
  ...
  #if defined (CONFIG_GT9XX_lianchuang_eep)
  			 if(XY_reverse) gtp_touch_down(input_x, GTP_MAX_HEIGHT-input_y, input_w, 0);                
                else gtp_touch_down(GTP_MAX_WIDTH-input_x, input_y, input_w, 0);
  #else       
            if ((id & 0x80)) 
			{
				if(!input_w)
	...
	GTP_DEBUG("id = %d,touch_index = 0x%x, pre_touch = 0x%x\n",id, touch_index,pre_touch);
	   for (i = 0; i < GTP_MAX_TOUCH; i++)
	   {
		...
		{   
			...
			input_y  = coor_data[pos + 3] | (coor_data[pos + 4] << 8);
			input_w  = coor_data[pos + 5] | (coor_data[pos + 6] << 8);
/*#if defined (CONFIG_GT9XX_lianchuang) || defined (CONFIG_GT9XX_lianchuang_eep)
						 if(XY_reverse)
						 	{
						 		gtp_touch_down(ts, id, input_x, GTP_MAX_HEIGHT-input_y, input_w);
						 		printk("905@@@@@@@@XY_reverse = 1\n");
						 	}
            				else 
            					{
            						//gtp_touch_down(ts, id, GTP_MAX_WIDTH-input_x, input_y, input_w);
            						gtp_touch_down(ts, id, GTP_MAX_HEIGHT-input_y, input_x, input_w);
            						printk("910@@@@@@@@XY_reverse = 0\n");
            					}
#else
            if(XY_reverse)
            	{
            		gtp_touch_down(ts, id,GTP_MAX_WIDTH-input_x, GTP_MAX_HEIGHT-input_y, input_w);
            		printk("916@@@@@@@@@1\n");
            	}
            else 
            	{
			...
			{
				if(y_reverse){
						if(xy_reverse) gtp_touch_down(ts, id, GTP_MAX_HEIGHT-input_y,GTP_MAX_WIDTH-input_x, input_w);
								else 	gtp_touch_down(ts, id,GTP_MAX_WIDTH-input_x, GTP_MAX_HEIGHT-input_y, input_w);						
						}else{
							if(xy_reverse) gtp_touch_down(ts, id, input_y,GTP_MAX_WIDTH-input_x, input_w);
								else 	gtp_touch_down(ts, id, GTP_MAX_WIDTH-input_x,input_y, input_w);
							}				
			}
			else{
				if(y_reverse){
						if(xy_reverse) gtp_touch_down(ts, id, GTP_MAX_HEIGHT-input_y,input_x, input_w);
								else 	gtp_touch_down(ts, id,input_x, GTP_MAX_HEIGHT-input_y, input_w);						
						}else{
							if(xy_reverse) gtp_touch_down(ts, id, input_y,input_x, input_w);
								else 	gtp_touch_down(ts, id, input_x,input_y, input_w);
							}		
			}
 ...
 */
    ...
    u8 *send_cfg_buf[] = {cfg_info_group1,
    ...
    //GTP_DEBUG("Config Groups\' Lengths: %d, %d, %d, %d, %d", 
...
Output:
    Executive outcomes. 
        0: succeed.
*******************************************************/
static unsigned int DeviceInfo_Val=0;

unsigned int Imt_TP_DeviceInfo(void)
{
    return DeviceInfo_Val;
}
EXPORT_SYMBOL(Imt_TP_DeviceInfo);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    of_property_read_u32(np, "x_reverse", &num1);
    x_reverse = num1;
    of_property_read_u32(np, "y_reverse", &num2);
    y_reverse = num2;
    of_property_read_u32(np, "xy_reverse", &num3);
    xy_reverse = num3;
    //printk("@@@@@@@@@x_reverse =  %d  y_reverse = %d  xy_reverse = %d \n",x_reverse,y_reverse,xy_reverse);
    //if(num==1)XY_reverse=1;
...
#ifdef CONFIG_GT9XX_lianchuang
        DeviceInfo_Val=12;
#endif

#ifdef CONFIG_GT9XX_lianchuang_eep
        DeviceInfo_Val=13;
#endif
    return 0;
...
//#ifdef GTP_CONFIG_OF
static const struct of_device_id goodix_ts_ids[] = {
