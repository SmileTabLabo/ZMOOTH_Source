...
#include <linux/irq.h>
#include "gt9xx.h"
//#include "../../huaqiang/mtk_gslX680.h"
...
#if GTP_ICS_SLOT_REPORT
    w=255-w;
    if(w==0)w++;//minimum pressure value is 1;
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (pre_touch || touch_num)
    {
		...
		if(XY_reverse) gtp_pen_down(input_x, GTP_MAX_HEIGHT-input_y, input_w, 0);                
                else gtp_pen_down(GTP_MAX_WIDTH-input_x, input_y, input_w, 0);
#else
        ...
        for (i = 0; i < GTP_MAX_TOUCH; i++)
        {
            ...
            if ((touch_index & (0x01<<i)))
            {
                ...
                if(XY_reverse)gtp_touch_down(ts, id,input_x, GTP_MAX_HEIGHT-input_y, input_w);
		else gtp_touch_down(ts, id,  GTP_MAX_WIDTH-input_x, input_y, input_w);
#else

                if(XY_reverse)gtp_touch_down(ts, id,GTP_MAX_WIDTH-input_x, GTP_MAX_HEIGHT-input_y, input_w);
...
#if defined (CONFIG_Huaqiang)
if(get_Pendix_preempt())  
	{
	//	printk("%s Pendix_preempt is on !!!\n",__func__);
		return IRQ_HANDLED;
	}
#endif
...
//#if GTP_DRIVER_SEND_CFG
    ...
    GTP_DEBUG("Config Groups\' Lengths: %d, %d, %d, %d, %d, %d", 
        ...
        cfg_info_len[4], cfg_info_len[5]);
            ...
            printk("CFG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X \n", sensor_id+1, 
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(Imt_LCD_DeviceInfo()==3)XY_reverse=1;
