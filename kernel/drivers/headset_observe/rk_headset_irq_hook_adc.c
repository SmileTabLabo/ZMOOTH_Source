...
#define HOOK_LEVEL_HIGH  		100//260		//1V*1024/2.5
#define HOOK_LEVEL_LOW  		50//204		//0.5V*1024/2.5
...
#if defined CONFIG_SND_SOC_RT5640
extern int rt5640_headset_mic_detect(bool jack_insert,bool force);
#endif
...
static irqreturn_t headset_interrupt(int irq, void *dev_id)
{
	...
	if(headset_info->headset_status == HEADSET_IN)
	{
		...
		//	if(headset_info->cur_headset_status==2)headset_info->cur_headset_status=1;
	...
	else if(headset_info->headset_status == HEADSET_OUT)
	{
		...
		if(headset_info->isMic)
		{
			...
			rt5640_headset_mic_detect(false,false);
			#endif
...
static void hook_once_work(struct work_struct *work)
{
	...
	#ifdef CONFIG_SND_SOC_RT5640
	rt5640_headset_mic_detect(true,false);
//	msleep(200);
	#endif
		...
		printk("hook_once_work read adc value: %d\n",val);
	...
	if(val >= 0 && val < HOOK_LEVEL_LOW)
	{
		...
		#ifdef CONFIG_SND_SOC_RT5640
		rt5640_headset_mic_detect(false,true);
		#endif				
	}	
		...
		if(val<10)
			{
				...
				ret = iio_read_channel_raw(headset->chan, &val);
				if (ret < 0) {
					pr_err("read hook adc channel() in HOOK error: %d\n", ret);
					goto out;
				}
				else
					DBG("hook_down check again adc value=%d\n",val);

				if(val<10)headset->hook_status = HOOK_DOWN;
