...
static struct rt5640_priv *rt5640_codec;
...
int rt5640_reset_P(void)
{
	printk("%s \n",__func__);
	return snd_soc_write(rt5640_codec->codec, RT5640_RESET, 0);
}
EXPORT_SYMBOL_GPL(rt5640_reset_P);
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		...
		snd_soc_write(codec, RT5640_PWR_ANLG1, 0x2000);
		snd_soc_write(codec, RT5640_PWR_ANLG2, 0x0800);
		break;
...
static bool BST1_val=0;
int rt5640_headset_mic_detect(bool jack_insert,bool force)
{
	unsigned int Reg_ANLG1_63,Reg_ANLG2_64;
	printk("%s jack_insert:%d force%d \n",__func__,jack_insert,force);
	if(rt5640_codec == NULL){
		return -1;
	}

	regmap_read(rt5640_codec->regmap,RT5640_PWR_ANLG1,&Reg_ANLG1_63);
	regmap_read(rt5640_codec->regmap,RT5640_PWR_ANLG2,&Reg_ANLG2_64);
	printk("%s Reg_ANLG1_63 0x%x ,Reg_ANLG2_64 0x%x \n",__func__,Reg_ANLG1_63,Reg_ANLG2_64);
	return 0;// set MICBIAS1 enable as codec boot on.

	if(jack_insert)
		{      
			BST1_val=0;
			if(Reg_ANLG2_64&0x800)BST1_val=1;
			else {
				Reg_ANLG1_63|= 0x2000;
				Reg_ANLG1_63|= 0x4;
				Reg_ANLG2_64|= 0x800;				
				regmap_write(rt5640_codec->regmap, RT5640_PWR_ANLG1, Reg_ANLG1_63);
				regmap_write(rt5640_codec->regmap, RT5640_PWR_ANLG2, Reg_ANLG2_64);
			}
	...
	else 
		{
			if(!BST1_val)
			{
				if(force)
					{
						regmap_update_bits(rt5640_codec->regmap,RT5640_PWR_ANLG1,RT5640_PWR_MB, 0);
						regmap_update_bits(rt5640_codec->regmap,RT5640_PWR_ANLG1,RT5640_PWR_LDO2, 0);
						regmap_update_bits(rt5640_codec->regmap,RT5640_PWR_ANLG2,RT5640_PWR_MB1, 0);						
					}
			}
		}
	regmap_read(rt5640_codec->regmap,RT5640_PWR_ANLG1,&Reg_ANLG1_63);
	regmap_read(rt5640_codec->regmap,RT5640_PWR_ANLG2,&Reg_ANLG2_64);
	printk("%s Reg_ANLG1_63 0x%x ,Reg_ANLG2_64 0x%x BST1_val:%d \n",__func__,Reg_ANLG1_63,Reg_ANLG2_64,BST1_val);

	msleep(20);
	return 0;
}
EXPORT_SYMBOL_GPL(rt5640_headset_mic_detect);
